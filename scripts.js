...
function calculateGeometric() {
    ...
    document.getElementById('unGeometric').value = un.toFixed(2);
    document.getElementById('snGeometric').value = sn.toFixed(2);

    let series = '';
    for (let i = 0; i < Math.min(n, 4); i++) {
        series += (a * Math.pow(r, i)).toFixed(2) + ', ';
    }
    if (n > 4) {
        series += '..., ' + (a * Math.pow(r, n - 1)).toFixed(2);
    }
